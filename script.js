...
function performSearch() {
  ...
  let matchedProducts = allProducts.map(prod => {
    let score = 0;
    let hitTags = [];
    let highlightIngredients = new Set();
    
    selectedTags.forEach(tag => {
      const normTag = normalize(tag);
      let isHit = false;

      // 1. 제품 설명이나 키 성분에 직접 포함되어 있는지 확인
      if (normalize(prod.description).includes(normTag)) {
        isHit = true;
      }
      
      prod.keyIngredients.forEach(ing => {
        if (normalize(ing).includes(normTag)) {
          isHit = true;
          highlightIngredients.add(ing);
        }
      });
      
      // 2. 효능/기능 검색: 성분 데이터를 기반으로 연결
      if (!isHit) {
        const matchedIngs = allIngredients.filter(i => 
          normalize(i.category).includes(normTag) || 
          normalize(i.role).includes(normTag) || 
          normalize(i.tag).includes(normTag) ||
          normalize(i.name).includes(normTag)
        );
        
        matchedIngs.forEach(matchedIng => {
          prod.keyIngredients.forEach(keyIng => {
            const nKey = normalize(keyIng);
            const nName = normalize(matchedIng.name);
            const nEn = normalize(matchedIng.englishName);
            if (nKey.includes(nName) || nName.includes(nKey) || (nEn && nEn.includes(nKey))) {
              isHit = true;
              highlightIngredients.add(keyIng);
            }
          });
        });
      }
    ...
    return { ...prod, score, hitTags, highlightIngredients: Array.from(highlightIngredients) };
  }).filter(p => p.score > 0);
  ...
  if (results.length === 0) {
    ...
    results.forEach(prod => {
      ...
      const tagsHtml = prod.keyIngredients.map(ing => {
        const isHit = prod.highlightIngredients ? prod.highlightIngredients.includes(ing) : prod.hitTags.some(t => ing.includes(t));
